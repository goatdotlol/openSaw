# Open Saw 0.1.0 release notes (draft)

## Summary

Open Saw `0.1.0` is the first public release-prep milestone for the current Rust implementation. Open Saw is Claude Code inspired and built as a clean-room Rust implementation; it is not a direct port or copy. This release centers on a usable local CLI experience: interactive sessions, non-interactive prompts, workspace tools, configuration loading, sessions, plugins, and local agent/skill discovery.

## Highlights

- Initial public `0.1.0` release candidate for Open Saw
- Safe-Rust implementation as the current primary product surface
- `saw` CLI for interactive and one-shot coding-agent workflows
...
```bash
saw
saw prompt "summarize this repository"
```
...
Position `0.1.0` as the first public release of Open Saw in its current Rust implementation for early adopters who are comfortable building from source. The feature surface is broad enough for real usage, while packaging and release automation can continue to improve in later releases.

## Verification used for this draft

- Workspace version verified from `Cargo.toml`
- `saw` binary/package path verified from `cargo metadata`
- CLI command surface verified from `cargo run --quiet --bin saw -- --help`
